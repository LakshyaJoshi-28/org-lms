  ...
  return (
    <div className="space-y-6 animate-fade-in pb-16">
      ...
      <div className="glass-panel p-6 rounded-3xl border border-slate-200 dark:border-slate-800/80 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <span className="text-[11px] font-bold text-amber-500 uppercase tracking-wider">Organization Governance</span>
          <h1 className="text-2xl font-black text-slate-900 dark:text-white mt-1">Certificates & Designer</h1>
        ...
        <div className="flex items-center space-x-2 bg-slate-100 dark:bg-slate-950/60 p-1.5 rounded-2xl border border-slate-200 dark:border-slate-800">
          <button
      ...
      {backfillCount > 0 && (
        <div className="glass-panel p-5 rounded-3xl border border-amber-500/30 bg-amber-500/5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 animate-scale-up">
          <div className="flex items-center space-x-3">
            <div className="w-10 h-10 rounded-2xl bg-amber-500/10 text-amber-500 border border-amber-500/20 flex items-center justify-center flex-shrink-0">
              <RefreshCw className={`w-5 h-5 ${runningBackfill ? 'animate-spin' : ''}`} />
            </div>
            <div>
              <h4 className="font-extrabold text-xs text-amber-700 dark:text-amber-400 uppercase tracking-wider flex items-center">
          ...
          <button
            ...
            className="px-5 py-2.5 rounded-2xl bg-gradient-to-r from-amber-500 to-orange-600 hover:from-amber-600 hover:to-orange-700 text-white text-xs font-extrabold shadow-lg shadow-amber-500/25 transition-all cursor-pointer disabled:opacity-50 flex-shrink-0"
          >
      ...
      {activeTab === 'issued' && (
        <div className="space-y-4">
          ...
          <div className="glass-panel p-4 rounded-2xl border border-slate-200 dark:border-slate-800 flex items-center space-x-3">
            ...
            />
            {search && (
              <button onClick={() => { setSearch(''); getOrgCertificates().then(res => setCertificates(res.data.data.certificates || [])); }} className="text-xs text-slate-400 hover:text-slate-600">
                Clear
              </button>
            )}
          </div>
      ...
      {activeTab === 'designer' && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {/* Settings Form */}
          <form onSubmit={handleSaveTemplate} className="glass-panel p-6 rounded-3xl border border-slate-200 dark:border-slate-800/80 space-y-5">
            <div className="border-b border-slate-200 dark:border-slate-800 pb-3">
              <h3 className="font-extrabold text-base text-slate-900 dark:text-white flex items-center">
                <Palette className="w-4 h-4 mr-2 text-amber-500" /> Template Styling
              </h3>
            ...
            <div className="p-3 rounded-2xl bg-amber-500/10 border border-amber-500/20 text-[11px] text-amber-700 dark:text-amber-300 space-y-1">
            ...
            <div className="pt-2 flex items-center justify-between gap-3">
              <button
                ...
                className="px-4 py-2.5 rounded-2xl border border-slate-300 dark:border-slate-700 text-slate-700 dark:text-slate-300 font-bold text-xs hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors cursor-pointer flex items-center"
              >
              ...
              <button
                type="submit"
                disabled={savingTemplate}
                className="px-6 py-2.5 rounded-2xl bg-amber-600 hover:bg-amber-500 text-white font-bold text-xs shadow-lg shadow-amber-500/20 transition-all cursor-pointer flex items-center"
              >
          ...
          <div className="lg:col-span-2 space-y-3">
            <div className="flex items-center justify-between px-2">
              ...
              <span className="text-[11px] text-emerald-500 font-bold">Real-time Rendering</span>
            </div>

            <div className="glass-panel p-4 rounded-3xl border border-slate-200 dark:border-slate-800/80">
              <CertificateCanvas
                templateSettings={template}
                sampleData={{
                  organizationName: 'IT360 Enterprise Organization',
